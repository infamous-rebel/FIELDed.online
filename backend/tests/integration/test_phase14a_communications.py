...
async def configured_biz(db_session: AsyncSession, biz_a: tuple[User, Business]) -> Business:
    ...
    await config_repo.upsert_channel_config(BusinessCommunicationChannel(
        business_id=biz.id, channel="EMAIL", enabled=True, provider_ref="resend",
    ))
    await config_repo.upsert_channel_config(BusinessCommunicationChannel(
        business_id=biz.id, channel="SMS", enabled=True, provider_ref="twilio",
    ))
    await config_repo.upsert_purpose_config(BusinessCommunicationPurpose(
        business_id=biz.id, purpose="TRANSACTIONAL", enabled=True,
    ))
    await config_repo.upsert_purpose_config(BusinessCommunicationPurpose(
        business_id=biz.id, purpose="MARKETING", enabled=True,
...
async def brain_with_version(db_session: AsyncSession, biz_a: tuple[User, Business]) -> BrainVersion:
    ...
    version = BrainVersion(
        brain_id=brain.id, version_number=1, status="active",
...
class TestSchemaVerification:
    ...
    async def test_communications_idempotency_unique(self, db_session: AsyncSession):
        result = await db_session.execute(text(
            "SELECT indexdef FROM pg_indexes WHERE tablename = 'communications'"
        ))
    ...
    async def test_webhook_provider_external_unique(self, db_session: AsyncSession):
        result = await db_session.execute(text(
            "SELECT indexdef FROM pg_indexes "
            "WHERE tablename = 'communication_webhooks'"
        ))
        indexes = [r[0] for r in result.all()]
...
class TestTenantIsolation:
    ...
    async def test_communication_tenant_scoped(
        ...
        comm = Communication(
            business_id=biz_a_obj.id, channel="EMAIL", purpose="TRANSACTIONAL",
            status="PENDING", idempotency_key=f"tenant-test-{uuid.uuid4().hex[:8]}",
        )
    ...
    async def test_template_tenant_scoped(
        ...
        template = CommunicationTemplate(
            business_id=biz_a_obj.id, channel="EMAIL", purpose="TRANSACTIONAL", name="T",
        )
    ...
    async def test_notification_tenant_scoped(
        ...
        n = Notification(
            business_id=biz_a_obj.id, notification_type="TEST", title="T", body="B",
    ...
    async def test_config_tenant_scoped(
        self, db_session: AsyncSession, biz_a: tuple, biz_b: tuple
    ):
        _, biz_a_obj = biz_a
        _, biz_b_obj = biz_b
        repo = CommunicationConfigRepository(db_session)
        await repo.upsert_channel_config(BusinessCommunicationChannel(
            business_id=biz_a_obj.id, channel="EMAIL", enabled=True,
        ))
...
class TestCommunicationConfiguration:
    ...
    async def test_upsert_channel_config(self, db_session: AsyncSession, biz_a: tuple):
        ...
        config2 = BusinessCommunicationChannel(
            business_id=biz.id, channel="EMAIL", enabled=False,
        )
    ...
    async def test_channel_unique_constraint(self, db_session: AsyncSession, biz_a: tuple):
        ...
        repo = CommunicationConfigRepository(db_session)
        await repo.upsert_channel_config(BusinessCommunicationChannel(
            business_id=biz.id, channel="EMAIL", enabled=True,
        ))
...
class TestConsentSuppression:
    ...
    async def test_opt_in_creates_preference(
        ...
        pref = CustomerCommunicationPreference(
            customer_id=customer.id, business_id=biz.id,
            channel="EMAIL", purpose="MARKETING",
            consent_state="OPTED_IN", opt_in=True,
            source="test", consented_at=datetime.now(timezone.utc),
        )
        result = await repo.upsert(pref)
        assert result.opt_in is True
        assert result.consent_state == "OPTED_IN"

    async def test_suppression_denies(
        self, db_session: AsyncSession, biz_a: tuple, customer: User
    ):
        _, biz = biz_a
        repo = ConsentRepository(db_session)
        pref = CustomerCommunicationPreference(
            customer_id=customer.id, business_id=biz.id,
            channel="EMAIL", purpose="MARKETING",
            consent_state="SUPPRESSED", opt_in=False,
            suppression=True, suppression_reason="Spam complaint",
        )
        result = await repo.upsert(pref)
        assert result.suppression is True
        assert result.suppression_reason == "Spam complaint"

    async def test_dnc_flag(
        self, db_session: AsyncSession, biz_a: tuple, customer: User
    ):
        _, biz = biz_a
        repo = ConsentRepository(db_session)
        pref = CustomerCommunicationPreference(
            customer_id=customer.id, business_id=biz.id,
            consent_state="DNC", opt_in=False, do_not_contact=True,
        )
        result = await repo.upsert(pref)
    ...
    async def test_upsert_updates_existing(
        ...
        pref1 = CustomerCommunicationPreference(
            customer_id=customer.id, business_id=biz.id,
            channel="EMAIL", purpose="MARKETING",
            consent_state="UNKNOWN", opt_in=False,
        )
...
class TestPolicyDecisions:
    ...
    async def test_missing_channel_config_returns_require_approval(
        ...
        decision = await policy.evaluate(
            ...
            recipient=RecipientContext(
                recipient_type="CUSTOMER", customer_id=customer.id,
                channel="NONEXISTENT", purpose="TRANSACTIONAL",
            ),
        )
    ...
    async def test_disabled_channel_returns_deny(
        ...
        await repo.upsert_purpose_config(BusinessCommunicationPurpose(
            business_id=biz.id, purpose="TRANSACTIONAL", enabled=True,
        ))
        policy = CommunicationPolicyService(db_session)
        decision = await policy.evaluate(
    ...
    async def test_transactional_bypasses_consent(
        self, db_session: AsyncSession, configured_biz: Business,
        brain_with_version: BrainVersion, customer: User
    ):
        policy = CommunicationPolicyService(db_session)
        decision = await policy.evaluate(
    ...
    async def test_marketing_with_opt_in_allowed(
        ...
        repo = ConsentRepository(db_session)
        await repo.upsert(CustomerCommunicationPreference(
            customer_id=customer.id, business_id=configured_biz.id,
            channel="EMAIL", purpose="MARKETING",
            consent_state="OPTED_IN", opt_in=True,
        ))
        policy = CommunicationPolicyService(db_session)
    ...
    async def test_suppressed_returns_deny(
        ...
        await repo.upsert(CustomerCommunicationPreference(
            customer_id=customer.id, business_id=configured_biz.id,
            channel="EMAIL", purpose="MARKETING",
            consent_state="SUPPRESSED", suppression=True,
    ...
    async def test_staff_bypasses_consent(
        ...
        decision = await policy.evaluate(
            business_id=configured_biz.id,
            recipient=RecipientContext(
                recipient_type="STAFF", channel="EMAIL", purpose="MARKETING",
    ...
    async def test_external_bypasses_consent(
        ...
        decision = await policy.evaluate(
            business_id=configured_biz.id,
            recipient=RecipientContext(
                recipient_type="EXTERNAL", channel="EMAIL", purpose="MARKETING",
...
class TestNotificationPersistence:
    ...
    async def test_create_notification(self, db_session: AsyncSession, biz_a: tuple, customer: User):
        ...
        n = Notification(
            ...
            notification_type="QUOTE_ISSUED", title="New Quote", body="Quote #123",
    ...
    async def test_unread_count(self, db_session: AsyncSession, biz_a: tuple, customer: User):
        ...
        for i in range(3):
            await svc.create_notification(
                business_id=biz.id, customer_id=customer.id,
                notification_type="TEST", title=f"T{i}", body="B",
...
class TestCommunicationPersistence:
    ...
    async def test_create_communication_with_recipient(
        ...
        recipient = CommunicationRecipient(
            communication_id=comm.id, recipient_type="CUSTOMER",
            channel="EMAIL", address="test@example.com", status="PENDING",
        )
    ...
    async def test_add_attempt(
        ...
        attempt = CommunicationAttempt(
            communication_id=comm.id, provider_name="resend",
            status="SUCCESS", provider_reference="msg_123",
        )
    ...
    async def test_communication_idempotency_key_unique(
        ...
        comm1 = Communication(
            business_id=biz.id, channel="EMAIL", purpose="TRANSACTIONAL",
    ...
    async def test_audit_event_append(
        self, db_session: AsyncSession, biz_a: tuple, customer: User
    ):
        _, biz = biz_a
...
class TestOutboxLifecycle:
    ...
    async def test_create_and_claim(
        ...
        event = OutboxEvent(
            business_id=biz.id, event_type="QUOTE_ISSUED",
            aggregate_type="quote", aggregate_id=uuid.uuid4(),
    ...
    async def test_mark_processed(self, db_session: AsyncSession, biz_a: tuple):
        ...
        repo = OutboxRepository(db_session)
        event = OutboxEvent(
            business_id=biz.id, event_type="TEST",
            aggregate_type="test", aggregate_id=uuid.uuid4(),
...
class TestDomainOutboxIntegration:
    ...
    async def test_quote_emits_outbox_event(
        ...
        await db_session.flush()
        offer = ServiceOffer(
            business_id=biz.id, category_id=cat.id,
            name=f"Svc-{uuid.uuid4().hex[:6]}", slug=f"svc-{uuid.uuid4().hex[:6]}",
        ...
        enquiry = Enquiry(
            ...
            customer_id=customer.id, business_id=biz.id,
            service_offer_id=offer.id, subject="Test", message="Test",
            status="quoted",
        )
        ...
        quote = Quote(
            ...
            customer_id=customer.id, business_id=biz.id,
            enquiry_id=enquiry.id, service_offer_id=offer.id,
            amount="100.00", currency="GBP", status="issued",
        )
        db_session.add(quote)
        await db_session.flush()
        # Verify outbox event was emitted
        repo = OutboxRepository(db_session)
        events = await repo.get_by_idempotency_key(
            f"QUOTE_ISSUED:quote:{quote.id}"
        )
    ...
    async def test_booking_emits_outbox_event(
        ...
        quote = Quote(
            ...
            customer_id=customer.id, business_id=biz.id,
            enquiry_id=enquiry.id, service_offer_id=offer.id,
            amount="100.00", currency="GBP", status="accepted",
        )
        db_session.add(quote)
        await db_session.flush()
        booking = Booking(
            ...
            customer_id=customer.id, business_id=biz.id,
            quote_id=quote.id, enquiry_id=enquiry.id,
            service_offer_id=offer.id,
            requested_at=datetime.now(timezone.utc) + timedelta(days=2),
            currency="GBP", status=BookingStatus.CONFIRMED,
        )
...
class TestCommunicationAPIAuthorization:
    ...
    async def test_unauthenticated_notifications_rejected(
    ...
    async def test_unauthenticated_request_rejected(
        self, client: AsyncClient, biz_a: tuple
    ):
        _, biz = biz_a
...
class TestWebhookPersistence:
    ...
    async def test_create_webhook(self, db_session: AsyncSession):
        ...
        webhook = CommunicationWebhook(
            provider="resend", external_event_id=f"evt-{uuid.uuid4().hex[:8]}",
    ...
    async def test_webhook_idempotency(self, db_session: AsyncSession):
        ...
        w1 = CommunicationWebhook(
            provider="resend", external_event_id=ext_id,
            event_type="email.delivered",
            received_at=datetime.now(timezone.utc),
        )
        ...
        w2 = CommunicationWebhook(
            provider="twilio_sms", external_event_id=ext_id,
    ...
    async def test_webhook_api_idempotent(
...
class TestTemplateLifecycle:
    ...
    async def test_create_template_with_version(
        self, db_session: AsyncSession, biz_a: tuple
    ):
        _, biz = biz_a
        repo = CommunicationTemplateRepository(db_session)
    ...
    async def test_multiple_versions(
        ...
        repo = CommunicationTemplateRepository(db_session)
        template = CommunicationTemplate(
            business_id=biz.id, channel="EMAIL", purpose="REMINDER",
            name="Reminder",
        )
        await repo.create(template)
        for i in range(1, 4):
            version = CommunicationTemplateVersion(
                template_id=template.id, version_number=i,
                subject=f"Reminder v{i}", body=f"Body v{i}",
            )
            await repo.add_version(version)
        versions = await repo.list_versions(template.id)
